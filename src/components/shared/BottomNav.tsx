        ...
        {navItems.map(({ href, label, icon }) => {
          ...
          return (
            <Link
              ...
              href={href}
              className={cn(
                'flex-1 flex flex-col items-center gap-0.5 py-2 px-1 transition-colors duration-200',
                active ? 'text-primary-700' : 'text-slate-400'
              )}
            >
              {icon(active)}
              <span className={cn('text-[10px] font-medium', active ? 'text-primary-700' : 'text-slate-400')}>
                {label}
              </span>
            </Link>
          );
